...
<div class="ui-main-container">
  ...
  <div class="ui-info-content">
	  <form id ="editMerch" method="post"  action="${action?if_exists}" >
	   <input type="hidden" name="chargetype" value="${chargetype?if_exists.chargeType}"/>
	     <input type="hidden" name="chargeTypeId" value="${chargetype?if_exists.chargeTypeId}"/>
			          ...
			          <tbody>
				          ...
				          <tr class="platform-part">
				           ...
				           <tr class="platform-part">
				            ...
				            <input type="hidden" name="chargeDesc" value="${domainitem?if_exists.chargeDesc}"/>
				          </tr>
		                     </#if>
                            </#list>
                                <#list providerChargeList  as provideritem> 
				            ...
				            <td class="ui-oddtd">备注：</td>
				            <td>
				              <input type="hidden" name="chargeDesc" value="${provideritem?if_exists.chargeDesc}"/>
		  ...
		  <div class="ui-submit-warpp">
	      <a href="javascript:history.go(-1);" class="ui-red-btn">返 回</a>
	      <!--<input type="submit" value="通过" class="ui-red-btn ui-pos3"/>-->
	       <input type="button" value="通过" onclick=domainok(${chargetype?if_exists.chargeType}); class="ui-red-btn ui-pos3"/>
	       <input type="button" value="拒绝" onclick=domainceanel(${chargetype?if_exists.chargeType},${chargetype?if_exists.chargeTypeId}); class="ui-red-btn ui-pos3"/>
	    </div>
		</form>
	</div>
</div>
...
<script>
 ...
 function domainceanel(value,chargeid){
      $.messager.prompt('提示消息', '确定要拒绝该笔审核记录吗？', function (r) {
                                 if (r.replace(/[ ]/g,"").length>0) {
                                     $.ajax({
                                         url: '/system/examine/cenceldown?chargeid='+chargeid+'&type='+value+'&desc='+r ,
                                         type: 'post',
                                         datatype: 'text',
                                         success: function (result) {
                                             if (result>0) {
                                             $.messager.alert("操作提示", "处理成功！", "info", function () {
                                               window.location.href="/system/examine/examView";
                                              });
                                             }else{
                                  ...
                                  $.messager.alert('提示消息', '请输入审核意见！', 'warn');	
                                 }
                             });
                         }
                          function domainok(value){
                                     $.ajax({
                                         url: '/system/examine/toInfoConfirm?type='+value,
                                         type: 'post',
                                         datatype: 'text',
                                         data: $('#editMerch').serialize(),
                                         success: function (result) {
                                             if (result.succ) {
                                             $.messager.alert("操作提示", "审核通过！", "info", function () {
                                               window.location.href="/system/examine/examView";
                                              });
                                             }else{
                                              $.messager.alert('提示消息', result.error.message, 'error');
                                             }
                                         }
                                     });
                         }
