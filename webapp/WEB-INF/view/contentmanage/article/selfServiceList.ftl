...
<div class="ui-main-container">
			...
			<#if cataList?if_exists>
				<#list cataList as cata>
					<li class="ui-menu-item ui-meun-item-open">
			 		    ...
			 		    <#--<#if cata?if_exists.id == 2>listView?pid=2&type=p<#else>javascript:;</#if>-->
						<a class="ui-menu-item-name parent_search" href="<#if cata?if_exists.id == 2>listView?pid=2&type=p<#else>javascript:;</#if>" data-id="${cata?if_exists.id}" title="${cata?if_exists.title}">${cata?if_exists.title}</a>
		...
		<div class="ui-list-content">
		  ...
		  <div class="search-item search-short" >
			<form name="searchform" method="get" action="" id="searchform" class="ui-search-form">
				<input id="" name="parentId" value="${parentId}" type="hidden"/>
				<input name="pid" value="${pid?if_exists}" type="hidden"/>
				<ul class="search-list fn-clear">
					<li>
						<span class="name">文章标题：</span>
						<span class="textbox"><input class="textbox-text textbox-prompt" maxlength=120  type="text" name="title" value="${title?if_exists}"/></span>
					</li>
					<li>
		  ...
		  <div class="list-item  nopage-list fn-clear">
		   ${fButton(list, 'ADD','<a href="@URL@?cataId=${pid?if_exists}" id="addArticleBut" class="ui-trans-btn ui-add2-pos">新增</a>')} 
			<#--<a id="addArticleBut" href="/content/article/addUI?cataId=${pid?if_exists}" class="ui-trans-btn ui-add2-pos">新增</a>-->
			${fButton(list, 'DELETE','<a href="javascript:delAll(\'userList\');"  class="ui-trans-btn ui-del-pos">删除</a>')} 
	...
	<div id="dlgConfirm" class="ui-dialog warn-dialog confirm-dialog">
	    ...
	    <div class="ui-dlg-text"><span id="del-tip">确定删除此文章吗？</span></div>
		   ...
		   <#--<a href="javascript:delNext('/content/article/delete','id');" class="ui-red-btn">确 定</a>-->
	...
	$("#userList").datagrid({
		...
		columns:[[
			...
			{field:'cataName',align:'center',width:"17%",title:'所属类别'},
			...
			{field:'opt',align:'center',width:"25%",title:'操作',
			    ...
			    <#--html='<a class="operation-link" href="/content/article/view?id='+rec.id+'">[查看]</a>'
			        +'<a class="operation-link" href="/content/article/editUI?id='+rec.id+'">[编辑]</a>'
			        +'<a class="operation-link" href="javascript:delConfirm('+rec.id+')">[删除]</a>';-->
			       return html;
				}
		    }
		]],
	...
	$(".submit_search").each(function(){
		$(this).click(function () {
			...
			var str= '{'+'"parentId":"' + id + '"'+ '}';
		    $("#userList").datagrid({ queryParams:$.parseJSON(str)  }); 
	...
	$(".parent_search").each(function(){
		$(this).click(function () {
			...
			if(id==1){//自足服务
		      ...
		      $("#userList").datagrid({ queryParams:form2Json('searchform')  }); 
		      $('#searchform').find('input[name="parentId"]:eq(0)').val(id);
			}
