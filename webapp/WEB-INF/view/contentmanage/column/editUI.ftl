...
<div class="ui-main-container">
	...
	<div  class="ui-info-container">
		<div class="ui-info-content">
	      <form id ="addMerchType" method="post" action="/content/column/editConfirm" >
	      	<input id="" name="id" value="${problemCata?if_exists.id?if_exists}" type="hidden"/>
			<div class="ui-info-wrap ">
		  	<h3 class="ui-info-tit">编辑栏目</h3>
				<div class="ui-infobg">
					<table class="ui-center-tab">
			          ...
			          <tr>
			            ...
			            <td> 
				            <#if problemCata?if_exists.id?if_exists>
				              <input type="hidden" data-unique="fieldId" value="${problemCata?if_exists.parentId}"/>
				            <#elseif problemCata?if_exists.cataId?if_exists>
				              <input type="hidden" data-unique="fieldId" value="${problemCata?if_exists.cataId}"/>
				            <#else>
				              <input type="hidden" data-unique="fieldId" id="chgeParent" value="1"/>
				            </#if>
			                <input type="hidden" data-first="${problemCata?if_exists.title}"/>
			            	<input type="text" name="title" id="title" maxlength=40 dataType="reqCol" value="${problemCata?if_exists.title?if_exists}" class="validate[required,custom[name],custom[typeName],ajax2[ajaxColumn],maxSize[40]] ui-dinput">
			            </td>
			          </tr>
			          <tr>
			            ...
			            <td>
				            ...
				            <#else>
				            	<#if problemCata?if_exists.cataId?if_exists>
					            	<#if problemCata?if_exists.cataId?if_exists == 1>自助服务</#if>
					            	<#if problemCata?if_exists.cataId?if_exists == 2>常见问题</#if>
					            	<input id="" name="parentId" value="${problemCata?if_exists.cataId?if_exists}" type="hidden"/>
					            	<#else>
			          ...
			          <tr>
			            ...
			            <td>
			            	<input type="text" name="sorts"  dataType="reqColNum"  class="validate[required,custom[integer],min[0],max[9999999999]] ui-dinput" value="${problemCata?if_exists.sorts?if_exists}"/>
			            </td>
			          </tr>
					</tbody>
				   </table>
...
<script>
  ...
  function chose_parent(obj){
    $('#chgeParent').val($(obj).val());
    $('#title').focus();
  }
</script>
</body>
