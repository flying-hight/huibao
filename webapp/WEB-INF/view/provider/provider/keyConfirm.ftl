				 ...
				 <#list keyList as item>
					<table class="ui-center-tab">
			          ...
			          <tr class="platform-part">
				           ...
				           <input type="hidden" name="providerKeyMap['${item?if_exists.keyId}'].publicKey" value="${item?if_exists.publicKey}"/>
				           <input type="hidden" name="providerKeyMap['${item?if_exists.keyId}'].keyId" value="${item?if_exists.keyId}"/>
				           <input type="hidden" name="providerKeyMap['${item?if_exists.keyId}'].effTimeStr" value="${item?if_exists.effTimeStr}"/>
				           <input type="hidden" name="providerKeyMap['${item?if_exists.keyId}'].expTimeStr" value="${item?if_exists.expTimeStr}"/>				          
		      		</table>
		      		</br>
		      		</#list>
				</div>
...
<div id="dlgOk2" class="ui-dialog ok2-dialog" data-options="iconCls:'icon-save'">
    ...
    <div class="ui-dlg-text"><#if provider.aests ==1>编辑密钥成功！<#else>新增密钥成功！</#if></div>
...
<div id="dlgWarn" class="ui-dialog warn-dialog"  data-options="iconCls:'icon-save'">
	<div class="ui-dlg-content ui-relative">
    <div class="ui-dlg-text"><#if provider.providerId ==1>编辑密钥失败！<#else>新增密钥失败！</#if></div>
