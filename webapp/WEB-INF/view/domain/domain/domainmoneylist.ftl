...
//用户类别管理
	$("#paramList").datagrid({
	   url:'/domain/findMoneyList',
		...
		pagePosition:top,
		columns:[[
			{field:'domainId',align:'center',width:"10%",title:'渠道ID' },
			{field:'domainName',align:'center',width:"12%",title:'渠道名称'}, 
			...
			{field:'domainSts',align:'center',width:"16%",title:'状态',
			formatter:function(value,rec){
					var s = '其他';
				if(rec.domainSts == 1){
					s = '待审核';
			...
			{field:'acctBalance',align:'center',width:"16%",title:'账户余额(元)',
			formatter:function(value,rec){
				var s = '-';
		 	if(rec.acctType == 2){
					s = rec.acctBalance;
			...
			{field:'acctFreezing',align:'center',width:"16%",title:'信用额度(元)',
			formatter:function(value,rec){
				var s = '-';
				if(rec.acctType == 1){
					s = rec.acctBalance;
			...
			{field:'opt',align:'center',width:"17%",title:'操作',formatter:function(value,rec){
			 var phtml='';
				  var  html=' ${fButton(list, 'RECHARGE','<a class="operation-link" href="@URL@?domainId=\'+rec.domainId+\'">[充值]</a>')}';
