			...
			<h3 class="ui-info-tit"> 渠道账户信息</h3>
					...
					<table class="ui-center-tab">
			          <tbody>
			          	<#if domain.domainId != null>
				            	<input id="getDomainId" type="hidden" name="domainId" value="${domain?if_exists.domainId}"/>
			  </#if>
				          ...
				          <#if domainAccount.acctType == 1&&domain.domainId != null> 
				          <tr class="platform-part">
				            <td class="ui-oddtd">信用额度：</td>
				            <td>
				                <input id='acctBalance' 　  type="text" maxlength=40 class="validate[required,custom[money]  ui-dinput"  name="acctBalance" value="${domainAccount?if_exists.acctBalance}"/>元
				            </td>
				          </tr></#if> 
				            <tr class="platform-part">
				            <td class="ui-oddtd">预警金额：</td>
				            <td>
				              <input id='ewBalance' 　  type="text" maxlength=40  class="validate[required,custom[money]  ui-dinput"  name="ewBalance" value="${domainAccount?if_exists.ewBalance}"/>元
				            </td>
				...
				<div class="ui-infobg">
				           ...
				           <tr class="platform-part">
				            ...
				            <td>
				              ...
				              <select class="ui-dselect" name="idenType">
				                	<option value="10" <#if domainIdentitie.idenType==10>selected</#if>>身份证</option>
				                	<option value="20" <#if domainIdentitie.idenType==20>selected</#if>>组织机构代码证</option>
				                	<option value="21" <#if domainIdentitie.idenType==21>selected</#if>>营业执照</option>
				                	<option value="22" <#if domainIdentitie.idenType==22>selected</#if>>税务登记证</option>
				                </select>
				            </td>
				            <td class="three">证件号码：</td>
				            <td>
				             <input id='idenNo' 　  type="text"  class="  ui-dinput" name="idenNo"    value="${domainIdentitie?if_exists.idenNo}"/>
				            </td>
				          </tr>
				  ...
				  <tr class="platform-part">
				            ...
				            <td colspan="3">
				            	   <input id="effTime" name="effTimeStr" class="easyui-datebox" value="${domain.effTime?string('yyyy-MM-dd')}" validType="mdM['#effTime']">
					<em>至</em>
					<input id="expTime" name="expTimeStr" class="easyui-datebox" value="${domain.expTime?string('yyyy-MM-dd')}" validType="mdM['#expTime']">
				            </td>
				          </tr> 
