...
<body>
  <div class="ui-main-container">
		...
		<form name="searchform" method="get" action="" id ="searchform" class="ui-search-form">
			<ul class="search-list fn-clear">
				...
				<li>
					...
					<span class="textbox"><input class="textbox-text textbox-prompt" type="text" name="username" value="${transactionMessDetailQuery?if_exists.username}"/></span>
				</li>
			...
			<li>
			   <span class="name">银行：</span>
				<span class="textbox"><input class="textbox-text textbox-prompt" type="text" name="institutionName" value="${transactionMessDetailQuery?if_exists.institutionName}"/></span>
			</li>
			<li>
			...
			<li>
				...
				<select class="seaway-combobox" name="state" >
					...
					<#if orderWithdrawState?has_content>
						<#list orderWithdrawState as cl>
			...
			<li>
				...
				<select class="seaway-combobox" name="auditState" >
					<option value="">全部</option>
						<#if auditState?has_content>
							<#list auditState as cl>
								<option <#if transactionMessDetailQuery?if_exists.state == cl.key?if_exists>selected="selected"</#if>value="${cl.key?if_exists}">${cl.value?if_exists}</option>
							</#list>
						</#if>						
				</select>
	...
	${fButton(list, 'EXPORTEXCEL','<a href="javascript:doExcel(\'@URL@\');" class="ui-excel ui-add-pos">[导出EXCEL表格]</a>')} 
		<#--<a href="javascript:doExcel('/report/transactionMess/withdrawDetailExcel');" class="ui-excel ui-add-pos">[导出EXCEL表格]</a>-->
	...
	$("#withdrawList").datagrid({
		url:'/report/transactionMess/selectWithdrawDetail',
		method:'get',
		fitColumns:true,
		rownumbers:true,
		...
		columns:[[
			...
			{field:'createTimeString',align:'center',width:"14%",title:'交易时间'},
			{field:'username',align:'center',width:"17%",title:'用户'},
			{field:'institutionName',align:'center',width:"10%",title:'银行'},
			{field:'withdrawSumStr',align:'center',width:"8%",title:'提现金额(元)'},
			{field:'auditStateStr',align:'center',width:"8%",title:'审核状态'},
			{field:'auditTime',align:'center',width:"10%",title:'审核时间'},
			{field:'institutionChannel',align:'center',width:"8%",title:'银行通道'},
			{field:'stateStr',align:'center',width:"8%",title:'交易状态'}
		]],
