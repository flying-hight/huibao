...
<head>
	...
	<title>统计报表-结算信息类-商户结算数据明细</title>
...
<body>
  <div class="ui-main-container">
		...
		<form name="searchform" method="get" action="" id ="searchform" class="ui-search-form">
			<ul class="search-list fn-clear">
				<li class="date">
					...
					<input id="startTime" name="beginDate" class="easyui-datebox" value="${tradeRecordQuery?if_exists.beginDate?if_exists}" validType="mdM['#endTime']">
					<em>至</em>
					<input id="endTime" name="endDate" class="easyui-datebox" value="${tradeRecordQuery?if_exists.endDate?if_exists}" validType="mdM['#startTime']">
				</li>
				<!-- 
				<li>
					<span class="name">商户：</span>
					<span class="textbox"><input class="textbox-text textbox-prompt" type="text" value=""/></span>
				</li> -->
				<li>
					<span class="name">商户：</span>
					<select class="seaway-combobox" name="sellerId" >
						<option value="">全部</option>
						<#if sellerList?has_content>
							<#list sellerList as cl>
								<option <#if tradeRecordQuery?if_exists.sellerId == cl.userId?if_exists>selected="selected"</#if> value="${cl.userId?if_exists}">${cl.sellerName?if_exists}</option>
							</#list>
						</#if>						
					</select>
				</li>			
	...
	${fButton(list, 'EXPORTEXCEL','<a href="javascript:doExcel(\'@URL@\');" class="ui-excel ui-add-pos">[导出EXCEL表格]</a>')} 
		<#--<a href="javascript:doExcel('/report/accountsMess/sellerAccountsDetailExcel');" class="ui-excel ui-add-pos">[导出EXCEL表格]</a>-->
	...
	$(function(){
	$("#userList").datagrid({
		url:'/report/accountsMess/selectsellerAcountsDetail',
		method:'get',
		fitColumns:true,
		rownumbers:true,
		...
		pagePosition:top,
		columns:[[
			{field:'orderNo',align:'center',width:"18%",title:'交易流水号'},
			{field:'sellerOrderNo',align:'center',width:"16%",title:'商户流水号'},
			{field:'successTimeStr',align:'center',width:"15%",title:'交易时间'},
			{field:'tradeType',align:'center',width:"13%",title:'交易类型'},
			{field:'tradeAmountStr2',align:'center',width:"12%",title:'交易金额(元)'},
			{field:'amountHandingStr',align:'center',width:"12%",title:'手续费'},
			{field:'amountFactStr',align:'center',width:"13%",title:'结算金额(元)'}
		]],
