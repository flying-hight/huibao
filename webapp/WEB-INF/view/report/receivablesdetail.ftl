...
<body>
  <div class="ui-main-container">
	...
	<div class="ui-list-content">
	  ...
	  <div class="search-item">
		<form name="searchform" method="get" action="" id ="searchform" class="ui-search-form">
					...
					<select class="seaway-combobox" name="sellerId" >
						...
						<#if sellerList?has_content>
							<#list sellerList as cl>
								<option <#if transactionMessDetailQuery?if_exists.sellerType == cl.userId?if_exists>selected="selected"</#if> value="${cl.userId?if_exists}">${cl.sellerName?if_exists}</option>
							</#list>
						</#if>						
					</select>
				</li>				
			...
			<li>
				<span class="name" style="width:75px;">商户流水号：</span>
				<span class="textbox"><input class="textbox-text textbox-prompt" type="text" name="merchOrderNo" value="${transactionMessDetailQuery?if_exists.merchOrderNo?if_exists}"/></span>
			</li>
		  </ul>
	...
	${fButton(list, 'EXPORTEXCEL','<a href="javascript:doExcel(\'@URL@\');" class="ui-excel ui-add-pos">[导出EXCEL表格]</a>')} 
		<#--<a href="javascript:doExcel('/report/transactionMess/receivablesDetailExcel');" class="ui-excel ui-add-pos">[导出EXCEL表格]</a>-->
		<table id="paymentList2" class="easyui-tab" width="100%">
		</table>
	...
	<script>
	$(function(){
	$("#paymentList2").datagrid({
		url:'/report/transactionMess/selectReceivablesDetail',
		method:'get',
		fitColumns:true,
		rownumbers:true,
		...
		columns:[[
			...
			{field:'productOrderNo',align:'center',width:"17%",title:'商户流水号'},
			{field:'createTimeString',align:'center',width:"17%",title:'交易时间'},
			{field:'sellerName',align:'center',width:"17%",title:'商户'},
			{field:'paymentSumStr',align:'center',width:"16%",title:'交易金额(元)'},
			{field:'stateStr',align:'center',width:"16%",title:'交易状态'}
		]],
    ...
    $("#paymentList2").datagrid("getPager").pagination({
		    pageSize:20,
		    displayMsg:"",
