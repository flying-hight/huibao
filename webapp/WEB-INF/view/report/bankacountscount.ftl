...
<head>
	...
	<title>统计报表-结算信息类-银行结算数据统计</title>
  ...
  <div class="ui-main-container">
	...
	<div class="list-item fn-clear ui-relative">
	  ${fButton(list, 'EXPORTEXCEL','<a href="javascript:doExcel(\'@URL@\');" class="ui-excel ui-add-pos">[导出EXCEL表格]</a>')} 
		<#--<a href="javascript:doExcel('/report/accountsMess/bankAcountsCountExcel');" class="ui-excel ui-add-pos">[导出EXCEL表格]</a>-->
	...
	<script>
	var ifSerach=false,tabhtml="",tabhead="",dataLen=0;
	$(function(){
	$("#userList").datagrid({
		url:'/report/accountsMess/selectBankAcountsCount',
		method:'get',
		fitColumns:true,
		rownumbers:true,
		...
		pagePosition:top,
		columns:[[
			{field:'countBank',align:'center',width:"9%",title:'银行'},
			{field:'countDayString',align:'center',width:"9%",title:'日期',
			  formatter:function(val,rec){
					       return dateformat(rec.countDayString);
				       }
			      },
			{field:'deductNum',align:'center',width:"9%",title:'扣款笔数'},
			{field:'deductSumYuan',align:'center',width:"9%",title:'扣款金额(元)'},
			{field:'paymentFundNum',align:'center',width:"9%",title:'退款笔数'},
			{field:'paymentFundSumYuan',align:'center',width:"9%",title:'退款金额(元)'},
			{field:'paymentNetAccountYuan',align:'center',width:"11%",title:'交易净额(元)'},
			{field:'entrustNum',align:'center',width:"10%",title:'委托结算笔数'},
			{field:'entrustSumYuan',align:'center',width:"10%",title:'委托结算金额(元)'},
			{field:'handingYuan',align:'center',width:"11%",title:'手续费(元)'},
			{field:'totalRevenueYuan',align:'center',width:"12%",title:'到账金额(元)'}
		]],
	...
	$("#submit_search").click(function () {
	    if($("#searchform").form("validate")){
	         ...
	         dateType();
		     ifSerach=true;
		     $("#userList").datagrid({ queryParams: form2Json("searchform") });   //点击搜索
	     }
	});
