			...
			<form name="searchform" method="get" action="" id ="searchform" class="ui-search-form">
				<ul class="search-list fn-clear">
					...
					<li>
						...
						<span class="textbox"><input class="textbox-text textbox-prompt textbox2" maxlength=120 type="text" name="tradeId" value=""/></span>
					</li>
					<li>
...
//用户类别管理
	$("#paramList").datagrid({
	   url:'/bussiness/bill/findList?serviceId=14',
		method:'get',
		fitColumns:true,
		rownumbers:true,
		...
		pagePosition:top,
		columns:[[
			{field:'tid',align:'center',width:"14%",title:'交易流水号'},
			...
			{field:'productParValue',align:'center',width:"7%",title:'面值价(元)'},
			 ...
			 {field:'tradeSts',align:'center',width:"13%",title:'状态',
				...
				var s = '其他';
				 if (rec.tradeSts==10) {s = '已下单';}
				 if (rec.tradeSts==-10) {s = '订单取消';}
				 if (rec.tradeSts==20) {s = '已支付';}
				 if (rec.tradeSts==21) {s = '支付不足';}
				 if (rec.tradeSts==-20) {s = '支付超时失败';}
				 if (rec.tradeSts==-21) {s = '支付不足超时失败';}
				 if (rec.tradeSts==30) {s = '发货中';}
				 if (rec.tradeSts==35) {s = '已发货';}
				 if (rec.tradeSts==-31) {s = '渠道余额不足发货失败';}
				 if (rec.tradeSts==-32) {s = '供应商余额不足发货失败';}
				 if (rec.tradeSts==-33) {s = '网络异常发货失败';}
				 if (rec.tradeSts==40) {s = '全部核销';}
				 if (rec.tradeSts==41) {s = '部分核销';}
				 if (rec.tradeSts==50) {s = '全部退款申请';}
				 if (rec.tradeSts==51) {s = '同意全部退款';}
				 if (rec.tradeSts==-51) {s = '拒绝全部退款';}
				 if (rec.tradeSts==55) {s = '部分退款申请';}
				 if (rec.tradeSts==56) {s = '同意部分退款';}
				 if (rec.tradeSts==-56) {s = '拒绝部分退款';}
		          return s;
			 }},
			{field:'opt',align:'center',width:"8%",title:'操作',formatter:function(value,rec){
				  var  html=' ${fButton(list, 'VIEW','<a class="operation-link" href="@URL@?id=\'+rec.tid+\'">[查看详情]</a>')}';
				   return  html;
