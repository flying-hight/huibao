...
<div class="ui-main-container">
	<div class="crumbs" id="crumbs">
	  ...
	  <div class="ui-info-content"> 
	    <form id ="addUserTypeNext" method="post" action="/userManage/userTypeManage/updateUserTypeOk" >     
		     ...
		     <#list userLevelExtendMdl.feeLevelExtendMdl as mdl>
				...
				<table class="ui-info-tab">
			      <tr>
			      	...
			      	<input type="hidden" name="feelLevelExtendMdl[<#if mdl_index?if_exists>${mdl_index}</#if>].operType" value="${mdl.operType}"/>
			      </tr>
			      <tr class="radio-wrap">
			      	<td class="ui-oddtd">手续费类型：</td>
			      	<td colspan='3' class="ui-eventd"> 
			      		${mdl.calculationMethodName} 
			      		<input type="hidden" name="feelLevelExtendMdl[<#if mdl_index?if_exists>${mdl_index}</#if>].calculationMethod" value="${mdl.calculationMethod}"/>
			      	</td>
			      </tr>
			      <tr class="ui-lastr">
			      	...
			      	<td colspan='3' class="ui-eventd fn-fee">
			      		...
			      		${mdl.maximumYuan} 元
			      		<input type="hidden" name="feelLevelExtendMdl[<#if mdl_index?if_exists>${mdl_index}</#if>].scale"  value="${mdl.scale}"/>
			      		<input type="hidden" name="feelLevelExtendMdl[<#if mdl_index?if_exists>${mdl_index}</#if>].minimumYuan"  value="${mdl.minimumYuan}"/>
			      	    <input type="hidden" name="feelLevelExtendMdl[<#if mdl_index?if_exists>${mdl_index}</#if>].maximumYuan"  value="${mdl.maximumYuan}"/>
			      	</td>
			      	<#else>
			      	<td colspan='3' class="ui-eventd fn-fee">
			      		${mdl.fixedYuan} 元
			      		<input type="hidden" name="feelLevelExtendMdl[<#if mdl_index?if_exists>${mdl_index}</#if>].fixedYuan"  value="${mdl.fixedYuan}"/>
			      	</td>
			      	</#if>
			      </tr>
				</table>
...
<div id="dlgOk2" class="ui-dialog ok2-dialog" data-options="iconCls:'icon-save'">
    ...
    <div class="ui-dlg-text">用户类别编辑成功！</div>
