...
<body>
  <div class="ui-main-container">
		...
		<input type="hidden" id="reviewPhase" name="reviewPhase"  value="${reviewPhase}"/>
			...
			<ul class="search-list fn-clear">
				...
				<li>
					...
					<span class="textbox"><input class="textbox-text textbox-prompt intOnly" maxlength=120 type="text" name="enterprisePhone" value=""/></span>
				</li>
	...
	<div class="list-item fn-clear ui-relative">
	   ...
	   <#-- <a href="javascript:doExcel('/userManage/userMessageAuth/enterpriseAuthListExcel/${reviewPhase}');" class="ui-excel ui-add-pos">[导出EXCEL表格]</a>-->
	...
	//用户信息查询
	$("#userList").datagrid({
		url:'/userManage/userMessageAuth/selectEnterpriseAuthList',
		method:'get',
		fitColumns:true,
		rownumbers:true,
		...
		columns:[[
			...
			{field:'realNameAuthReviewStatus',align:'center',width:"8%",title:'状态',sortable:true,
			
			 formatter:function(val,rec){
			         return rec.realNameAuthReviewStatusName;
			       }
			...
			{field:'opt',align:'center',width:"11%",title:'操作',
				formatter:function(value,rec){
					...
					if((${reviewPhase}==0&&sta==10)||(${reviewPhase}==1&&sta==20)||(${reviewPhase}==2&&sta==30)){
						...
						//return '<a class="operation-link" href="/userManage/userMessageAuth/selectEnterpriseAuthDetails/${reviewPhase}/?id='+rec.userId+'">[审核]</a>';
					}else{
						  return '';
					}
