...
<body>
  <div class="ui-main-container">
	...
	<div class="ui-list-content">
		...
		${fButton(list, 'ALLUP','<a class="ui-trans-btn fn-left ui-add3-pos"  onclick="alldownproduct()">批量上架</a>')}
...
//用户类别管理
	$("#paramList").datagrid({
	   url:'/product/findList?productSts=9',
		method:'get',
		fitColumns:true,
		rownumbers:true,
		...
		columns:[[
 		...
 		return html;
			...
			{field:'opt',align:'center',width:"15%",title:'操作',formatter:function(value,rec){
				  var  html=' ${fButton(list, 'UP','<a style="cursor:pointer;" class="operation-link" onclick="downproduct(\'+rec.pid+\')">[上架]</a>')}';
				  var  phtml=' ${fButton(list, 'EDIT','<a class="operation-link" href="@URL@?productid=\'+rec.pid+\'">[编辑]</a>')}';
				   return  html+phtml;
...
function alldownproduct(){
           var ids = [];
            ...
            if(ids.length==0){
            $.messager.alert('提示消息', '请勾选需要上架商品！', 'warn');
            }else{
             $.messager.confirm('提示消息', '确定要上架已选商品？', function (r) {
                                 if (r) {
                                     $.ajax({
                                         url: '/product/alldown?ids='+ids+'&sts=1' ,
                                         type: 'post',
                                         datatype: 'text',
                                         success: function (result) {
                                             if (result>0) {
                                               $.messager.alert("操作提示", "处理成功！", "info", function () {
                                               window.location.href="/product/downListView";
                                              });
                                             }else{
...
function downproduct(value){
             $.messager.confirm('提示消息', '确定要上架商品？商品ID' +value, function (r) {
                                 if (r) {
                                     $.ajax({
                                           url: '/product/alldown?ids='+value+'&sts=1' ,
                                         type: 'post',
                                         datatype: 'text',
                                         success: function (result) {
                                              if (result>0) {
