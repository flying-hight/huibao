...
<#function fCrumbs list>
    ...
    <#if list?has_content>
      <#list list as list>
        ...
        <#if list_has_next>
         <#assign html=html+'<a data-id="${list.resourceId}" onclick="window.parent.control.powerPage(this)" href="${list.url}">${(list.name)?if_exists?substring(str)}</a><em>></em>'>
        <#else>
          <#assign html=html+'<span data-id="${list.resourceId}" class="crumbs-tit" >${(list.name)?if_exists?substring(str)}</span>'>
        </#if>
      </#list>
    </#if>
    <#return html>
...
<script>
   ...
   { 
    ...
    var levelThree=$("#crumbs").find(".crumbs-tit").eq(0).data("id");
