...
<body style="">
	...
	$(function(){
		...
		//列表
		$("#dg-list").datagrid({
			url:'${base}/businessmanager/salesmoneyAjax?productId=${productId}&beginDate=${beginDate}&endDate=${endDate}',
			method:'post',
			fitColumns:true,
			rownumbers:true,
			...
			columns:[[
				...
				{field:'payAmount',align:'center',width:"48%",title:'销售金额(元)',
					formatter:function(value,rec){
						return eb.fenToThousands(value);
					}
				},
		   ...
		   loadFilter:function(data){
					...
					'<em class="query-result-num">${productName}</em>在各分销渠道的销售金额明细如下：</span>'+
					'</div>';
