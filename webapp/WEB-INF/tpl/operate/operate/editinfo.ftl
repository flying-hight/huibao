...
<body>
     ...
     <h2 class="ui-info-tit">产品信息</h2>
        <ul class="ui-info-list">
         	<li class="ui-info-item ui-info-item-grey">
         		<label class="ui-info-lbl">产品ID</label>
         		  ${product.productId}         		   
         	    <label class="ui-info-lbl ui-name-mr">保险名称</label>
      ...
      <h2 class="ui-info-tit">配置信息</h2>
        ...
        <li class="ui-info-item ui-info-item-grey">
         	...
         	<li class="ui-info-item ui-info-item-grey">
         		<label class="ui-info-lbl">分配数量</label>
         		   <input   name="productId" type="hidden" value="${domainproduct.productId}">
         		     <input   name="domainId" type="hidden" value="${product.domainId}">
	    ...
	    <div id="dosome" class="ui-btn-double ui-detail-btnposition">
	      <input type="button" value="编辑" onclick="editpuduct()" class="ui-btn-blue" />
	       <#if domainproduct.domainsts==2>
	      <input type="button" value="下架"  onclick="downproduct( ${product.productId},${product.domainId})" class="ui-btn-blue" />
	      </#if>
  ...
  <div id="dlgmq" class="easyui-dialog ui-dlg-alert"  data-options="iconCls:'icon-save',modal:true,closed: true" style="width:360px;">
    ...
    <input  type="hidden" name="issuedomain"   id="issuedomain"/>
  ...
  <div id="editlos" class="easyui-dialog ui-dlg-alert"  data-options="iconCls:'icon-save',modal:true,closed: true" style="width:360px;">
   ...
   <div id="dlgtime" class="easyui-dialog ui-dlg-alert"  data-options="iconCls:'icon-save',modal:true,closed: true" style="width:360px;">
	...
	function saveproduct(){
	 ...
	 $.ajax({
                 url: '${base}/operate/updateProduct',
                 type: 'post',
                 datatype: 'text',
	 ...
	 function tomain(){
          window.location.href="${base}/operate/productView";
   }
   
   function downproduct(value,domainid){
    $("#issueType").val(value);
    $("#issuedomain").val(domainid);
   showAlert("dlgmq");
   }
   function submitselect(){
	var ids = $("#issueType").val();
	var domainid = $("#issuedomain").val();
	 $.ajax({
	                 url: '${base}/operate/downProduct?ids='+ids+'&domainid='+domainid,
	                 type: 'post',
	                 datatype: 'text',
	                 success: function (result) {
	                     if (result>0) {
