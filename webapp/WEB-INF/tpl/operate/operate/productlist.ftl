...
<body style="">
		...
		<div class="ui-datagrid-wrap">
			...
			<div class="search-wrap">
				<form  id="search-ff" class="ui-search-form">
					<div class="search-specialbox">
						<div class="search-specialbox-tit">请先选择产品上架渠道：</div>
						<div>
						  <#list dList as item>
							<input type="radio" class="search-radioitem" name="domainId" onclick="submitForm()"  value="${item.domainId}" > ${item.domainName}
							 </#list>
						</div>
						<div class="search-specialbox-tit">请选择该渠道要上架的产品：</div>
					</div>
					<ul class="search-list fn-clear">
						<li>
							<label class="name">产品ID</label>
							<span class="textbox"><input class="textbox-text textbox-prompt intOnly" type="text" name="productId" value="" maxlength=9/></span>
						</li>
						<li>
 ...
 <div id="dlgmq" class="easyui-dialog ui-dlg-alert"  data-options="iconCls:'icon-save',modal:true,closed: true" style="width:360px;">
    ...
    <input  type="hidden" name="issueType"   id="issueType"/>
     <input  type="hidden" name="hiddendomain"   id="hiddendomainid"/>
  </div>
	<script>
	function celdiv(){
	...
	$(function(){
		...
		//列表
		$("#dg-list").datagrid({
			url:'${base}/operate/findoperateProduct',
			method:'post',
			fitColumns:true,
			rownumbers:true,
			...
			columns:[[
				...
				{field:'productId',align:'center',width:"9%",title:'产品ID'},
				{field:'productName',align:'center',width:"22%",title:'保险名称'},
				{field:'insurerName',align:'center',width:"12%",title:'承保公司'},
				{field:'shengyuQuantity',align:'center',width:"10%",title:'剩余数量(份)',sortable:true,
   ...
   function sureselect(){
            var ids = [];
            var domainid=$(".search-radioitem"),
            	domainval = '';
            
            $.each(domainid,function(i,v){
            	if(v.checked){
            		domainval = v.value;
            	}
            })
              $("#hiddendomainid").val(domainval);
...
var domainid = $("#hiddendomainid").val();
window.location.href="${base}/operate/selectUpProduct?ids="+ids+"&domainid="+domainid;
}
