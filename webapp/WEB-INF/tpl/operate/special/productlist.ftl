...
<body style="">
				...
				<form  id="search-ff" class="ui-search-form">
					<ul class="search-list fn-clear">
						<li>
							...
							<span class="textbox"><input class="textbox-text textbox-prompt intOnly"  onKeyUp="this.value=this.value.replace(/\D/g,'') " onafterpaste="this.value=this.value.replace(/\D/g,'')" type="text" name="productId" value=""  maxlength=9 /></span>
						</li>
						<li>
 ...
 <div id="dlgmq" class="easyui-dialog ui-dlg-alert"  data-options="iconCls:'icon-save',modal:true,closed: true" style="width:360px;">
    <h3 class="ui-dlg-tit">确定为已选产品新增专题吗？</h3>
	...
	$(function(){
		...
		//列表
		$("#dg-list").datagrid({
			url:'${base}/operate/findSpecialProductView',
			method:'post',
			fitColumns:true,
			rownumbers:true,
			...
			columns:[[
				...
				{field:'productName',align:'center',width:"10%",title:'保险名称'},
				{field:'insurerName',align:'center',width:"10%",title:'承保公司'},
				{field:'shengyuQuantity',align:'center',width:"10%",title:'剩余数量(份)'},
				{field:'minPremium',align:'center',width:"10%",title:'最低保费(元)',sortable:true,
					formatter: function(value,rec){
    	                return rec.minPremium+"-"+rec.maxPremium;
               		}},
    ...
    onLoadSuccess:function(){
      		pageCls();
      		sortCell(['maxPremium','minPremium']);//排序要显示箭头的，以数组形式传
      	}
		});
		//分页
		pageAction();
...
window.location.href="${base}/operate/selectSpecialProduct?ids="+ids;
}
